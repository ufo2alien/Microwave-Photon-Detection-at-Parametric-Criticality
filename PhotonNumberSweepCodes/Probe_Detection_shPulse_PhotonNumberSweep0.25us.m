...
MeasName = ['Probe_Detection_shPulse_PhotonNumberSweep',filename];
...
atten=0;
NAdet=-0.00e6;
FIXpump=0.54;0.67;0.365;0.76;%[linspace(-41,-6,36) linspace(-5,7,21) linspace(7.2,13,25) linspace(13,20.5,11)]-atten;%[linspace(-41,-6,36) linspace(-5,5,21) linspace(5.2,10,50) linspace(10.5,20.5,21)]-atten;%linspace(-70,-25,11) [linspace(-70,-25,11) linspace(-24,20,21)];%-70;%[linspace(-70,-25,21) linspace(-24,20,45)]%[-70 -55 -45 -30 -25 -20 linspace(-15,25,41)];
probePowerRaw=[0.001 0.005 logspace(-1.7,-0.1,201)];%linspace(-40,10,41);
...
h_p = 6.62607004e-34;
VoltageGeneratorV = 5.78;0.741;0.729;0.729;%0.7392;
...
pulseLength=5e-6;
pumpLength=3e-6;
probeLength=0.25e-6;%tau
probeDelay=1e-6+0.228e-6;
t1=0.5e-6+probeDelay;
t2=t1+probeLength+0.5e-6;

%Threshold

ThresMean=5.41;3.174;4;
ThresVar=12.28;9.623;14.5;
...
[ExpPulse,Energy,ExpPulseStr] = ArbSQUAREPulse(1,1000*probeLength,250e6,250e6*pulseLength+1,1,250e6*probeLength+1,MeasFreq);
[ExpPulse2,Energy2,ExpPulseStr2] = ArbSQUAREPulse(1,20000e-6,250e6,250e6*pulseLength+1,250e6*1e-6+1,250e6*pumpLength+1,MeasFreq*2);
...
fprintf(TriggerGenerator,['TRIG2:DEL ' num2str(probeDelay)]);
...
IQ_raw=IQ_raw./n_ave;
    ...
    if probeState==2
        HPProbeOn(1,probePowerI)=mean(HPProbeOnStat(probePowerI,:),2);
        LPProbeOn(1,probePowerI)=mean(LPProbeOnStat(probePowerI,:),2);
%         HPProbeOnSTD(1,probePowerI)=2*std(HPProbeOnStat(probePowerI,:),2);
%         LPProbeOnSTD(1,probePowerI)=2*std(LPProbeOnStat(probePowerI,:),2);
    elseif probeState==1
        LPProbeOff(1,probePowerI)=mean(LPProbeOffStat(probePowerI,:),2);
        HPProbeOff(1,probePowerI)=mean(HPProbeOffStat(probePowerI,:),2);
        HPProbeOffSTD(1,probePowerI)=2*std(HPProbeOffStat(probePowerI,:));
        LPProbeOffSTD(1,probePowerI)=2*std(LPProbeOffStat(probePowerI,:));
    end
...
if probePowerI==length(probePower)
     save([TempDirPath 'Data\' MeasName,'_' num2str(probeState) '_' num2str(probePower(probePowerI))  '_' num2str(VoltageGeneratorV) '.mat']);
end
if (db2pow(probePower(1,probePowerI)-101.4-30)*Energy./(h_p.*PumpGenFreqCent/2)<1.2)&&(db2pow(probePower(1,probePowerI)-101.4-30)*Energy./(h_p.*PumpGenFreqCent/2)>0.8)
...
clf;
subplot(2,2,1);
semilogx(db2pow(probePower(1,1:probePowerI)-101.3-30)*Energy./(h_p.*PumpGenFreqCent/2), [HPProbeOn(1,1:probePowerI).' HPProbeOff(1,1:probePowerI).']);
...
% clf;
subplot(2,2,2);
semilogx(db2pow(probePower(1,1:probePowerI)-101.3-30)*Energy./(h_p.*PumpGenFreqCent/2), [LPProbeOn(1,1:probePowerI).' LPProbeOff(1,1:probePowerI).']);
...
grid on;
subplot(2,2,3);
semilogx(db2pow(probePower(1,1:probePowerI)-101.3-30)*Energy./(h_p.*PumpGenFreqCent/2), [HPProbeOn(1,1:probePowerI).'-HPProbeOff(1,1:probePowerI).' LPProbeOn(1,1:probePowerI).'-LPProbeOff(1,1:probePowerI).']);
% semilogx(db2pow(probePower(1,1:probePowerI)-116-10)*2e-5./(h_p.*PumpGenFreqCent/2), [HPProbeOn(1:probePowerI).'./HPProbeOff(1:probePowerI).' LPProbeOn(1:probePowerI).'./LPProbeOff(1:probePowerI).']);
...
grid on;
subplot(2,2,4);
semilogx(db2pow(probePower(1,1:probePowerI)-101.3-30)*Energy./(h_p.*PumpGenFreqCent/2), [HPProbeOffSTD(1,1:probePowerI).' LPProbeOff(1,1:probePowerI).']);
% semilogx(db2pow(probePower(1,1:probePowerI)-116-10)*2e-5./(h_p.*PumpGenFreqCent/2), [HPProbeOn(1:probePowerI).'./HPProbeOff(1:probePowerI).' LPProbeOn(1:probePowerI).'./LPProbeOff(1:probePowerI).']);
...
% clf;
% subplot(2,2,3)
semilogx(db2pow(probePower(1,1:probePowerI)-101.3-30)*Energy./(h_p.*PumpGenFreqCent/2), [ ClickProbVarTrue(1,1:probePowerI).'  ClickProbVarFalse(1,1:probePowerI).' ClickProbMeanTrue(1,1:probePowerI).'  ClickProbMeanFalse(1,1:probePowerI).'],'Linewidth',3);
