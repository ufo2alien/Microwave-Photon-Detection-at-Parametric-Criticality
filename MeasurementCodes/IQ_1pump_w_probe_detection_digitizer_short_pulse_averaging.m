...
atten=0;
NAdet=-0.00e6;
FIXpump=0.560;0.67;0.365;0.76;%[linspace(-41,-6,36) linspace(-5,7,21) linspace(7.2,13,25) linspace(13,20.5,11)]-atten;%[linspace(-41,-6,36) linspace(-5,5,21) linspace(5.2,10,50) linspace(10.5,20.5,21)]-atten;%linspace(-70,-25,11) [linspace(-70,-25,11) linspace(-24,20,21)];%-70;%[linspace(-70,-25,21) linspace(-24,20,45)]%[-70 -55 -45 -30 -25 -20 linspace(-15,25,41)];
probePowerRaw=logspace(-2.3,-1,101);%linspace(-40,10,41);
...
h_p = 6.62607004e-34;
VoltageGeneratorV = 0.739;0.741;0.729;0.729;%0.7392;
...
t2=0.5e-6+0.25e-6+probeLength+probeDelay;
...
[ExpPulse,Energy,ExpPulseStr] = ArbExpPulse(1,1000*probeLength,250e6,250e6*pulseLength+1,125+1,MeasFreq);
[ExpPulse2,Energy2,ExpPulseStr2] = ArbExpPulse(1,1000e-6,250e6,250e6*pulseLength+1,250e6*pulseLength/2+1,MeasFreq*2);
...
IQ_raw=IQ_raw./n_ave;
    ...
    B1(:,cycle_i)=IQ_cap(1,round((cycle_i-1)*2*pulseLength.*samplerate)+(round(t1.*samplerate):round(t2.*samplerate))).';%IQfiltering(IQ_cap(1,round(t1.*samplerate):round(t2.*samplerate)),samplerate,0,1,[-25e6 25e6]).';
...
% end
    ...
    if ii==1
        subplot(1,2,1);
        cla;
        plot(t_s(round(t1.*samplerate):round(t2.*samplerate)),[real(A1(:,1)) imag(A1(:,1))]);
%         hold on
%         plot([t1 t1],ylim,'--g');
%          plot([t2 t2],ylim,'--g');
%          hold off
...
if ii==2
HPProbeOn(1,probePowerI)=var(A1(:,1));
LPProbeOn(1,probePowerI)=mean(A1(:,1));
elseif ii==1
LPProbeOff(1,probePowerI)=mean(A1(:,1));
HPProbeOff(1,probePowerI)=var(A1(:,1));
end
...
clf;
subplot(2,2,1);
semilogx(db2pow(probePower(1,1:probePowerI)-91.85-30)*Energy./(h_p.*PumpGenFreqCent/2), [HPProbeOn(1,1:probePowerI).' HPProbeOff(1,1:probePowerI).']);
...
% clf;
subplot(2,2,2);
semilogx(db2pow(probePower(1,1:probePowerI)-91.85-30)*Energy./(h_p.*PumpGenFreqCent/2), [LPProbeOn(1,1:probePowerI).' LPProbeOff(1,1:probePowerI).']);
