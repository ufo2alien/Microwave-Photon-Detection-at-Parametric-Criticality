...
MeasName = ['Probe_Detection_shPulse_2nd',filename];
...
atten=0;
NAdet=-0.00e6;
FIXpump=0.45;0.75;0.40;0.67;0.365;%[linspace(-41,-6,36) linspace(-5,7,21) linspace(7.2,13,25) linspace(13,20.5,11)]-atten;%[linspace(-41,-6,36) linspace(-5,5,21) linspace(5.2,10,50) linspace(10.5,20.5,21)]-atten;%linspace(-70,-25,11) [linspace(-70,-25,11) linspace(-24,20,21)];%-70;%[linspace(-70,-25,21) linspace(-24,20,45)]%[-70 -55 -45 -30 -25 -20 linspace(-15,25,41)];
probePowerRaw=logspace(-3,0,401);%linspace(-40,10,41);
probePower=probeVppCalibration(probePowerRaw);
...
h_p = 6.62607004e-34;
VoltageGeneratorV = 0.736;0.7395;0.741;0.729;%0.7392;
...
[ExpPulse,Energy,ExpPulseStr] = ArbExpPulse(1,500e-9,250e6,250e1,MeasFreq);
...
fprintf(TriggerGenerator,['SOUR2:VOLT:OFFS 0.0067']);
...
HPProbeOn=zeros(1,length(probePower));
HPProbeOff=zeros(1,length(probePower));
LPProbeOn=zeros(1,length(probePower));
LPProbeOff=zeros(1,length(probePower));
...
fprintf(TriggerGenerator,'OUTP2 ON');
    elseif ii==1
% fprintf(TriggerGenerator,['SOUR2:VOLT ' num2str(-55.5)]);
...
mainfig=figure(11442);
clf;
subplot(2,2,1)
semilogx(db2pow(probePower(1,1:probePowerI)-121.07)*Energy./(h_p.*PumpGenFreqCent/2), [HPProbeOn(1,1:probePowerI).' HPProbeOff(1,1:probePowerI).']);
...
% clf;
subplot(2,2,2)
semilogx(db2pow(probePower(1,1:probePowerI)-121.07)*Energy./(h_p.*PumpGenFreqCent/2), [LPProbeOn(1,1:probePowerI).' LPProbeOff(1,1:probePowerI).']);
...
grid on
subplot(2,2,3)
semilogx(db2pow(probePower(1,1:probePowerI)-121.07)*Energy./(h_p.*PumpGenFreqCent/2), [HPProbeOn(1,1:probePowerI).'-HPProbeOff(1,1:probePowerI).' LPProbeOn(1,1:probePowerI).'-LPProbeOff(1,1:probePowerI).']);
% semilogx(db2pow(probePower(1,1:probePowerI)-116-10)*2e-5./(h_p.*PumpGenFreqCent/2), [HPProbeOn(1:probePowerI).'./HPProbeOff(1:probePowerI).' LPProbeOn(1:probePowerI).'./LPProbeOff(1:probePowerI).']);
