...
MeasName = ['Probe_Detection_shPulse',filename];
...
atten=0;
NAdet=-0.00e6;
FIXpump=0.68 ;0.67;0.365;0.76;%[linspace(-41,-6,36) linspace(-5,7,21) linspace(7.2,13,25) linspace(13,20.5,11)]-atten;%[linspace(-41,-6,36) linspace(-5,5,21) linspace(5.2,10,50) linspace(10.5,20.5,21)]-atten;%linspace(-70,-25,11) [linspace(-70,-25,11) linspace(-24,20,21)];%-70;%[linspace(-70,-25,21) linspace(-24,20,45)]%[-70 -55 -45 -30 -25 -20 linspace(-15,25,41)];
probePowerRaw=logspace(-2.2,-0.7,31);%linspace(-40,10,41);
...
h_p = 6.62607004e-34;
VoltageGeneratorV = 0.738;0.741;0.729;0.729;%0.7392;
...
[ExpPulse,Energy,ExpPulseStr] = ArbExpPulse(1,1000*probeLength,250e6,250e6*pulseLength+1,250e6*probeLength+1,MeasFreq);
[ExpPulse2,Energy2,ExpPulseStr2] = ArbExpPulse(1,20000e-6,250e6,250e6*pulseLength+1,250e6*pulseLength/2+1,MeasFreq*2);
...
clf;
subplot(2,2,1);
semilogx(db2pow(probePower(1,1:probePowerI)-91.7-30)*Energy./(h_p.*PumpGenFreqCent/2), [HPProbeOn(1,1:probePowerI).' HPProbeOff(1,1:probePowerI).']);
...
% clf;
subplot(2,2,2);
semilogx(db2pow(probePower(1,1:probePowerI)-91.7-30)*Energy./(h_p.*PumpGenFreqCent/2), [LPProbeOn(1,1:probePowerI).' LPProbeOff(1,1:probePowerI).']);
...
grid on;
subplot(2,2,3);
semilogx(db2pow(probePower(1,1:probePowerI)-91.85-30)*Energy./(h_p.*PumpGenFreqCent/2), [HPProbeOn(1,1:probePowerI).'-HPProbeOff(1,1:probePowerI).' LPProbeOn(1,1:probePowerI).'-LPProbeOff(1,1:probePowerI).']);
% semilogx(db2pow(probePower(1,1:probePowerI)-116-10)*2e-5./(h_p.*PumpGenFreqCent/2), [HPProbeOn(1:probePowerI).'./HPProbeOff(1:probePowerI).' LPProbeOn(1:probePowerI).'./LPProbeOff(1:probePowerI).']);
...
grid on;
subplot(2,2,4);
semilogx(db2pow(probePower(1,1:probePowerI)-91.85-30)*Energy./(h_p.*PumpGenFreqCent/2), [HPProbeOffSTD(1,1:probePowerI).' LPProbeOff(1,1:probePowerI).']);
% semilogx(db2pow(probePower(1,1:probePowerI)-116-10)*2e-5./(h_p.*PumpGenFreqCent/2), [HPProbeOn(1:probePowerI).'./HPProbeOff(1:probePowerI).' LPProbeOn(1:probePowerI).'./LPProbeOff(1:probePowerI).']);
...
savefig(mainfig,[TempDirPath 'Figures\' MeasName,'_VarsMeans_' num2str(probeLength) 'ns.fig'])
%% Close devices
