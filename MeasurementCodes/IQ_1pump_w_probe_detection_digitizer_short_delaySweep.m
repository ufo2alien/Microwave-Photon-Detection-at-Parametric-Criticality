...
probePowerRaw=0.0747;logspace(-2.5,-1,21);%linspace(-40,10,41);
...
h_p = 6.62607004e-34;
VoltageGeneratorV = 5.8;0.741;0.729;0.729;%0.7392;
...
probeDelay=linspace(0e-6,2.5e-6,335);0.206e-6;
...
tot_meas_all=length(probeDelay).*(n_ave);
tot_meas_done=0;
t_ar=[];
tic
...
for probeDelayI=1:length(probeDelay)
    fprintf(TriggerGenerator,['TRIG2:DEL ' num2str(probeDelay(probeDelayI))]);
    t1=1e-6+0.203e-6;
    t2=t1+1.5e-6;
    ClickVarTrue=0;
    ClickVarFalse=0;
...
IQ_raw=IQ_raw./n_ave;
    ...
    %     title('PROBE ON')
      ...
      if HPProbeOnStat(probeDelayI,cycle_i)>=ThresVar
          ClickVarTrue=ClickVarTrue+1;
      end
      
      
      if LPProbeOnStat(probeDelayI,cycle_i)>=ThresMean
...
%         HPProbeOnSTD(1,probeDelayI)=2*std(HPProbeOnStat(probePowerI,:),2);
