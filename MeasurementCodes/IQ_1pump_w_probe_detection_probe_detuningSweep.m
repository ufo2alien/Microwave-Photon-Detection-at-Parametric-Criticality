...
MeasName = ['Probe_Detection_detuningSweep_1us_',filename];
...
atten=0;
NAdet=-0.00e6;
FIXpump=0.52;0.67;0.365;0.76;%[linspace(-41,-6,36) linspace(-5,7,21) linspace(7.2,13,25) linspace(13,20.5,11)]-atten;%[linspace(-41,-6,36) linspace(-5,5,21) linspace(5.2,10,50) linspace(10.5,20.5,21)]-atten;%linspace(-70,-25,11) [linspace(-70,-25,11) linspace(-24,20,21)];%-70;%[linspace(-70,-25,21) linspace(-24,20,45)]%[-70 -55 -45 -30 -25 -20 linspace(-15,25,41)];
...
probeDetuning=linspace(-3e6,3e6,601);
...
tot_meas_all=length(probeDetuning).*(n_ave);
tot_meas_done=0;
t_ar=[];
tic
...
for probeDetuningI=1:length(probeDetuning)
    
    [ExpPulse,Energy,ExpPulseStr] = ArbExpPulse(1,1000*probeLength,250e6,250e6*pulseLength+1,1,250e6*probeLength+1,MeasFreq+probeDetuning(probeDetuningI));
    fprintf(TriggerGenerator,['SOUR2:APPL:ARB']);
    fprintf(TriggerGenerator,['SOUR2:DATA:VOL:CLE']);
    pause(0.2);
    fprintf(TriggerGenerator,['SOUR2:DATA:ARB EXPDEC, ' ExpPulseStr]);
...
IQ_raw=IQ_raw./n_ave;
    ...
    %     view(0,90);
    %     title('PROBE ON')
      HPProbeOnStat(probeDetuningI,cycle_i)=var(B1(:,cycle_i));
      LPProbeOnStat(probeDetuningI,cycle_i)=abs(mean(B1(:,cycle_i)));
      if HPProbeOnStat(probeDetuningI,cycle_i)>=ThresVar
          ClickVarTrue=ClickVarTrue+1;
      end
      
      
      if LPProbeOnStat(probeDetuningI,cycle_i)>=ThresMean
    ...
    elseif probeState==1
                    ...
                    %     view(0,90);
                    %     title('PROBE OFF')
     HPProbeOffStat(probeDetuningI,cycle_i)=var(B1(:,cycle_i));
     LPProbeOffStat(probeDetuningI,cycle_i)=abs(mean(B1(:,cycle_i)));
     if HPProbeOffStat(probeDetuningI,cycle_i)>=ThresVar
          ClickVarFalse=ClickVarFalse+1;
     end
     
     if LPProbeOffStat(probeDetuningI,cycle_i)>=ThresMean
    ...
    ClickProbVarTrue(probeDetuningI)=ClickVarTrue/N_cycles;
    ClickProbVarFalse(probeDetuningI)=ClickVarFalse/N_cycles;
    ClickProbMeanTrue(probeDetuningI)=ClickMeanTrue/N_cycles;
    ClickProbMeanFalse(probeDetuningI)=ClickMeanFalse/N_cycles;    
    
    if probeState==2
        HPProbeOn(1,probeDetuningI)=mean(HPProbeOnStat(probeDetuningI,:),2);
        LPProbeOn(1,probeDetuningI)=mean(LPProbeOnStat(probeDetuningI,:),2);
...
%         LPProbeOnSTD(1,probePowerI)=2*std(LPProbeOnStat(probePowerI,:),2);
    elseif probeState==1
        LPProbeOff(1,probeDetuningI)=mean(LPProbeOffStat(probeDetuningI,:),2);
        HPProbeOff(1,probeDetuningI)=mean(HPProbeOffStat(probeDetuningI,:),2);
        HPProbeOffSTD(1,probeDetuningI)=2*std(HPProbeOffStat(probeDetuningI,:));
        LPProbeOffSTD(1,probeDetuningI)=2*std(LPProbeOffStat(probeDetuningI,:));
    end
...
% clf;
% subplot(2,2,3)
plot(probeDetuning(1,1:probeDetuningI), [ ClickProbVarTrue(1,1:probeDetuningI).'  ClickProbVarFalse(1,1:probeDetuningI).' ClickProbMeanTrue(1,1:probeDetuningI).'  ClickProbMeanFalse(1,1:probeDetuningI).'],'Linewidth',3);
