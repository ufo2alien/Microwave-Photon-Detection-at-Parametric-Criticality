...
MeasName = ['PhotonPulse_Detection_ROC',filename];
...
atten=0;
NAdet=-0.00e6;
FIXpump=0.40;0.67;0.365;0.76;%[linspace(-41,-6,36) linspace(-5,7,21) linspace(7.2,13,25) linspace(13,20.5,11)]-atten;%[linspace(-41,-6,36) linspace(-5,5,21) linspace(5.2,10,50) linspace(10.5,20.5,21)]-atten;%linspace(-70,-25,11) [linspace(-70,-25,11) linspace(-24,20,21)];%-70;%[linspace(-70,-25,21) linspace(-24,20,45)]%[-70 -55 -45 -30 -25 -20 linspace(-15,25,41)];
probePowerRaw=0.0347;0.025;%logspace(-2.7,-1.3,201);%linspace(-40,10,41);
...
h_p = 6.62607004e-34;
VoltageGeneratorV = 5.77;0.741;0.729;0.729;%0.7392;
...
pulseLength=5e-6;
pumpLength=3e-6;
probeLength=1e-6;%tau
probeDelay=1e-6+0.223e-6;
t1=0.5e-6+probeDelay;
t2=t1+probeLength+0.5e-6;

%Threshold

ThresMean=linspace(0.1,25,301);4;
ThresVar=linspace(1,200,301);14.5;
...
[ExpPulse,Energy,ExpPulseStr] = ArbExpPulse(1,1000*probeLength,250e6,250e6*pulseLength+1,1,250e6*probeLength+1,MeasFreq);
...
ClickVarTrue=zeros(1,length(ThresVar));
ClickVarFalse=zeros(1,length(ThresVar));
ClickMeanTrue=zeros(1,length(ThresMean));
ClickMeanFalse=zeros(1,length(ThresMean));
    for probeState=1:2%TURN OFF-ON probe signal

        if probeState==2
            fprintf(TriggerGenerator,['SOUR2:VOLT ' num2str(probePowerRaw(probePowerI))]);
            fprintf(TriggerGenerator,'OUTP2 ON');
        elseif probeState==1
            fprintf(TriggerGenerator,['SOUR2:VOLT ' num2str(0.001)]);
...
%         end
            ...
            for ThresVal_i=1:length(ThresVar)
                ...
                if probeState==1
                 ...
                 HPProbeOffStat(probePowerI,cycle_i)=var(B1(:,cycle_i));
                 LPProbeOffStat(probePowerI,cycle_i)=abs(mean(B1(:,cycle_i)));
                 if HPProbeOffStat(probePowerI,cycle_i)>=ThresVar(ThresVal_i)
                      ClickVarFalse(ThresVal_i)=ClickVarFalse(ThresVal_i)+1;
                 end

                 if LPProbeOffStat(probePowerI,cycle_i)>=ThresMean(ThresVal_i)
                      ClickMeanFalse(ThresVal_i)=ClickMeanFalse(ThresVal_i)+1;
                 end

                elseif probeState==2

                  HPProbeOnStat(probePowerI,cycle_i)=var(B1(:,cycle_i));
                  LPProbeOnStat(probePowerI,cycle_i)=abs(mean(B1(:,cycle_i)));

                  if HPProbeOnStat(probePowerI,cycle_i)>=ThresVar(ThresVal_i)
                      ClickVarTrue(ThresVal_i)=ClickVarTrue(ThresVal_i)+1;
                  end

                  if LPProbeOnStat(probePowerI,cycle_i)>=ThresMean(ThresVal_i)
                      ClickMeanTrue(ThresVal_i)=ClickMeanTrue(ThresVal_i)+1;
                  end

                end
                end
                TPR_Var(probePowerI,ThresVal_i)=ClickVarTrue(ThresVal_i)/N_cycles;
                FPR_Var(probePowerI,ThresVal_i)=ClickVarFalse(ThresVal_i)/N_cycles;
                TPR_Mean(probePowerI,ThresVal_i)=ClickMeanTrue(ThresVal_i)/N_cycles;
                FPR_Mean(probePowerI,ThresVal_i)=ClickMeanFalse(ThresVal_i)/N_cycles;   
            end 


        
%          savefig(hhh,[TempDirPath 'Figures\' MeasName,'_' num2str(probePower(probePowerI)) '_' num2str(VoltageGeneratorV) '.fig'])
...
AUC_var=trapz(flip(FPR_Var),flip(TPR_Var));
ROCfig=figure(11432);
...
subplot(1,2,2)
% cla
plot(FPR_Var(1,1:end),TPR_Var(1,1:end),'LineWidth',4);
title(['ROC - Detection by Variance. AUC: ' num2str(AUC_var)]);
xlabel('False Positive Rate');
ylabel('True Positive Rate');
xlim([0 1]);
ylim([0 1]);
...
disp(trapz(flip(FPR_Mean),flip(TPR_Mean)));
disp(trapz(flip(FPR_Var),flip(TPR_Var)));

% probabfig=figure(1146);
% semilogx(db2pow(probePower(1,1:probePowerI)-91.85-30)*Energy./(h_p.*PumpGenFreqCent/2), [ TPR_Var(1,1:probePowerI).'  FPR_Var(1,1:probePowerI).' TPR_Mean(1,1:probePowerI).'  FPR_Mean(1,1:probePowerI).'],'Linewidth',3);
% title ('Probability of Clicks and False clicks');
% legend('Variance TRUE','Variance FALSE', 'Mean TRUE', 'Mean FALSE');
...
savefig(ROCfig,[TempDirPath 'Figures\' MeasName,'_Probabilities_' num2str(VoltageGeneratorV) 'V_' num2str(probeLength*1e6) 'us.fig']);
save([TempDirPath 'Data\' MeasName, '_' num2str(probeState) '_' num2str(probePower(probePowerI))  '_' num2str(VoltageGeneratorV) '.mat'], '-regexp', '^(?!(IQ|A1|IQ_init|AAA|B1|L1|B11|A0|L|t_s|dem|IQ_raw|g|G|h|H)$).');
