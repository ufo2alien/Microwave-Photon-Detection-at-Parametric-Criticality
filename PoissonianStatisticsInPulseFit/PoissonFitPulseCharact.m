...
load('D:\Kirill\QWJPA_v2_2\11-Jun-2021\Probe_Detection_shPulse_PhotonNumberSweep202106111331\ProbeCharacter1us.mat');
%% Read data
...
figure(559)
clf
plot(ThresVal,[coDark(1,:,1).' 1-coEff(:,1) ]);%(coEff(30,:,2).'-coDark(:,2))./(1-coDark(:,2))
%  hold on
...
c2=0.1;
clear co coEff coDark stepHist
% TPR_Mean=0.5313;
% FPR_Mean=0.1158;
...
for ind=1:indMax
for ThresValI=1:length(ThresVal)
    stepHist(ThresValI,:)=[min(ThresVal) ThresVal(ThresValI) max(ThresVal) 100];
    coDark(ind,ThresValI,:)=histc(abs(LPProbeOffStat(ind,:)),stepHist(ThresValI,:))/length(LPProbeOffStat(ind,:));
end
end

for ind=1:indMax
    for ThresValI=1:length(ThresVal)
         co(ind,ThresValI,:)=histc(abs(LPProbeOnStat(ind,:)),stepHist(ThresValI,:))/length(LPProbeOnStat(ind,:));
    end
end
...
for ThresValI=1:length(ThresVal)
   ...
   coEff(ThresValI,:)=histc(abs(LPProbeOnStat(find(phNum>=1,1,'first'),:)),stepHist(ThresValI,:))/length(LPProbeOnStat(82,:));
...
[X,Y]=meshgrid(exp(-(coeff_nPois.*phNum(1:ind))),-log(1-(TPR_Mean-FPR_Mean)./(1-FPR_Mean)));%-log(1-(TPR_Mean-FPR_Mean)./(1-FPR_Mean)))
coef=1;

[ZZ,Z]=meshgrid((phNum(1:ind)),1-(TPR_Mean-FPR_Mean)./(1-FPR_Mean));%-coDark(1,:,2))./(1-coDark(1,:,2)));%(TPR_Mean-FPR_Mean)./(1-FPR_Mean))
% Z=squeeze(1-coEff(:,:,1)).';
...
s=surf(ax1,1*phNum(1:ind),ThresVal(1:end),co(:,:,1).'+0*co(:,:,2).');
view(ax1,0,90)
xlim(ax1,[0 10])
zlim(ax1,[0 1])
caxis(ax1,[0 1])
...
hold on
%     figure(556)
[scont,scont1]=contour3(ax2,phNum(1:ind),ThresVal(1:end),(X).^(Y.^(1./(c2.*ZZ+1))).*(squeeze(coDark(:,:,1)).'),linspace(0,1,21),'Linewidth',3,'ShowText','on','LabelSpacing',700);
% set(scont)
linkaxes=[ax1,ax2];
ax2.Visible = 'off';
ax2.XTick = [];
...
zlim(ax2,[0 1])
caxis(ax2,[0 1])
clabel(ax2,scont,scont1,'FontSize',14);
colormap(ax1,'hot')
colormap(ax2,'cool')
set([ax1,ax2],'Position',[.17 .11 .685 .72]);
cb1 = colorbar(ax1,"east");
cb2 = colorbar(ax2,"north");
...
colorbar north
view(0,90)
 set(ax2,'FontSize',18,'FontName','Segoe UI Semibold');
% set(s,'FaceAlpha',0.6)
% set(s,'EdgeColor','none')
xlabel(ax1,'$\left<n\right>$ in 1 pulse','interpreter','latex')
ylabel(ax1,'Threshold value','interpreter','latex')
zlabel(ax1,'$p(0)$','interpreter','latex')
...
grid on
set(gca,'FontSize',14);
xlabel('mean photon number in pulse $<n>$','interpreter','latex')
...
xlabel('<n> in block');
ylabel(['switching events in each block, ' num2str(length(LPProbeOnStat)/M) ' max'])
title (['number of blocks M = ' num2str(M)])
colorbar;
set(s,'EdgeColor','none')
colormap hot
...
% hold on
clf
mv=plot(phNum*length(LPProbeOnStat)/M,[meanM1.' varM1.' meanMs.' varMs.']);
